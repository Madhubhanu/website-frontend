...
const Dashboard = () => {
  ...
  const [uploadedPdfs, setUploadedPdfs] = useState([]); // Store uploaded PDFs
    ...
    const fetchBatches = async () => {
      try {
        const response = await fetch(`https://website-backend-r1z2.onrender.com/get-batchesF`);
...
useEffect(() => {
  ...
  const fetchReportStatus = async () => {
    try {
      const response = await fetch(`https://website-backend-r1z2.onrender.com/get-report-status?batch_name=${selectedBatch}`);
...
const handlePdfUpload = async (event) => {
  ...
  try {
    const response = await fetch(`https://website-backend-r1z2.onrender.com/upload-pdf?batch_name=${selectedBatch}&patient_id=${selectedPatient}`, {
      method: "POST",
      body: formData,
    });
 ...
 const response = await fetch("https://website-backend-r1z2.onrender.com/excel-download", {
      method: "POST",
  ...
  const fetchDataFromAPIs = async () => {
    try {
      ...
      const responseConditionsData = await fetch(
        `https://website-backend-r1z2.onrender.com/get-batch-data?batch_name=${selectedBatch}`
      );
      ...
      const responseData = await fetch(
        `https://website-backend-r1z2.onrender.com/get-batch-data2?batch_name=${selectedBatch}`
      );
      if (!responseData.ok) {
